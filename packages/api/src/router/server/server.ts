...
export const serverRouter = router({
      ...
      protectedMutationFetchFirst({
        async operation() {
          ...
          try {
            ...
            const isValidDomain = validDomainRegex.test(newCustomDomain) || newCustomDomain === "";
            if (!isValidDomain) {
              throw new TRPCError({
                code: "BAD_REQUEST",
            ...
            if (oldServerInfo.customDomain && oldServerInfo.customDomain !== newCustomDomain) {
  ...
  verifyCustomDomain: withUserServersProcedure.input(z.string()).query(async ({ input }) => {
    const domain = input;
  ...
  fetchDashboardById: withUserServersProcedure.input(z.string()).query(async ({ input, ctx }) =>
    protectedFetch({
      fetch: async () => {
  ...
  fetchPageViewsAsLineChart: withUserServersProcedure.input(z.string()).query(({ input, ctx }) => {
