...
let bot_discord_account_router: ReturnType<(typeof discordAccountRouter)["createCaller"]>;
let bot_ignored_discord_account_router: ReturnType<
...
beforeEach(async () => {
  ...
  bot_discord_account_router = discordAccountRouter.createCaller(bot);
  bot_ignored_discord_account_router = ignored_discord_account_router.createCaller(bot);
  await bot_discord_account_router.create(discord_account);
});
...
describe("Ignored Discord Account Operations", () => {
  describe("Ignored Discord Account Upsert", () => {
    it("should upsert an ignored discord account as the answer overflow bot", async () => {
      const result = await bot_ignored_discord_account_router.upsert(discord_account.id);
  ...
  describe("Ignored Discord Account By Id", () => {
    it("should find an ignored discord account by id as the answer overflow bot", async () => {
      await bot_ignored_discord_account_router.upsert(discord_account.id);
      const result = await bot_ignored_discord_account_router.byId(discord_account.id);
      expect(result).toEqual({ id: discord_account.id });
    });
    it("should fail to find an ignored discord account by id that is not the user", async () => {
      await testAllSources({
        async operation(source) {
          ...
          const { ctx } = await mockAccountCallerCtx(source);
          await bot_ignored_discord_account_router.upsert(account.id);
          const router = ignored_discord_account_router.createCaller(ctx);
  ...
  describe("Ignored Discord Account Many", () => {
    it("should find many ignored discord accounts as the answer overflow bot", async () => {
      await bot_ignored_discord_account_router.upsert(discord_account.id);
      await bot_ignored_discord_account_router.upsert(discord_account_2.id);
      const result = await bot_ignored_discord_account_router.byIdMany([
        discord_account.id,
    ...
    it("should fail to find many ignored discord accounts that are not the user", async () => {
      await testAllSources({
        async operation(source) {
          ...
          const { ctx } = await mockAccountCallerCtx(source);
          await bot_ignored_discord_account_router.upsert(account.id);
          await bot_ignored_discord_account_router.upsert(account2.id);
          const router = ignored_discord_account_router.createCaller(ctx);
  ...
  describe("Stop Ignoring Discord Accounts", () => {
    it("should stop ignoring a discord account as the answer overflow bot", async () => {
      await bot_ignored_discord_account_router.upsert(discord_account.id);
      const result = await bot_ignored_discord_account_router.stopIgnoring(discord_account.id);
      expect(result).toEqual({ id: discord_account.id });
    });
