...
// TODO: Fix
const getBaseUrl = () => {
	if (typeof window !== 'undefined') return `http://localhost:3000`; // browser should use relative url
				...
				links: [
					...
					httpBatchLink({
						...
						fetch(url, opts) {
							return fetch(url, {
								...opts,
								// We're doing this for multi tenancy
								// TODO: Maybe don't include if on the main domain?
								credentials: 'include',
							});
						},
					}),
