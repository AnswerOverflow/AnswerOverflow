...
export const buttonStyles: ButtonStylesType = {
	...
	black: {
		...
		ghost: {
			default:
				'bg-transparent text-black border-1 border-black hover:bg-[#070A0D]/5',
...
export const convertToCva = (styles: ButtonStylesType): CompoundVariants[] => {
	...
	for (const [key, value] of Object.entries(styles) as Entries<typeof styles>) {
		for (const [key2, value2] of Object.entries(value) as Entries<
			typeof value
		>) {
			for (const [key3, value3] of Object.entries(value2) as Entries<
				typeof value2
			>) {
				if (key3 === 'default') {
					cvaStyles.push({
