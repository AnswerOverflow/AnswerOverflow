  ...
  toXml() {
    ...
    const urls = this.entries.map(
      (entry) => {
        ...
        data += `<loc>${entry.loc.startsWith('http') ? entry.loc : this.baseUrl + entry.loc}</loc>`;
        if (entry.lastmod) data += `<lastmod>${entry.lastmod.toISOString()}</lastmod>`;
        if (entry.changefreq && this.type === 'url') data += `<changefreq>${entry.changefreq}</changefreq>`
        data += this.type === 'sitemap' ? '</sitemap>' : '</url>'
        return data;
      },
    );
