...
It's extremely hard to index content in a regular text channel with no threads as it lacks an indication of where the conversation starts and ends. Auto threading creates a thread for every message in a channel, making it easier to keep discussions on track. You may want to use this in a channel where you want a lighter weight version of a forum channel.
...
Use `/channel-settings` in the channel you want to disable auto thread in. Click the **{OPEN_HELP_CHANNEL_UTILITIES_LABEL}** menu followed by clicking **{DISABLE_AUTO_THREAD_LABEL}**.
