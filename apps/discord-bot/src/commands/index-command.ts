...
function handleIndexDMCommand(message: Message) {
...
function handleIndexAll(message: Message) {
	return Effect.gen(function* () {
		const discord = yield* Discord;
...
function handleIndexServer(message: Message, serverId: string) {
	return Effect.gen(function* () {
		const discord = yield* Discord;
...
function handleIndexStatus(message: Message) {
	return Effect.gen(function* () {
		const discord = yield* Discord;
	...
	Effect.gen(function* () {
		...
		yield* discord.client.on("messageCreate", (message) =>
			Effect.gen(function* () {
				...
				yield* handleIndexDMCommand(message);
			}),
		);
	}),
);
