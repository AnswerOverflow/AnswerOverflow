...
import { mockForumChannel, mockPublicThread, mockTextChannel } from '@answeroverflow/discordjs-mock';
...
describe('Send mark solution instructions', () => {
  it("should not send mark solution instructions if thread is not newly created", async () => {
    await expect(sendMarkSolutionInstructionsInThread(textChannelThread, false)).rejects.toThrowError(
  ...
  it("should not send mark solution instructions if thread does not have a parent channel", async () => {
    ...
    await expect(sendMarkSolutionInstructionsInThread(textChannelThread, true)).rejects.toThrowError(
  ...
  it("should successfully send mark solution instructions if thread is newly created", async () => {
    ...
    const instructionEmbed = sentMessage?.embeds.find((embed) => embed.description?.includes('help others find answers, you can mark'));
    expect(instructionEmbed).toBeDefined();
    expect(instructionEmbed?.image?.url).toBe('https://cdn.discordapp.com/attachments/1020132770862874704/1025906507549790208/mark_solution_instructions.png');
  });
});
