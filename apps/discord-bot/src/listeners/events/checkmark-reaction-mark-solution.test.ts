...
describe('Checkmark Reaction Mark Solution', () => {
	it('should not mark a message as a solution if the emoji is not a checkmark', async () => {
		...
		const preEventTextChannelMessagesLength =
			textChannelThread.messages.cache.size;
		await emitEvent(
			client,
		...
		const postEventTextChannelMessagesLength =
			textChannelThread.messages.cache.size;
		expect(postEventTextChannelMessagesLength).toBe(
			preEventTextChannelMessagesLength,
		);
	});
