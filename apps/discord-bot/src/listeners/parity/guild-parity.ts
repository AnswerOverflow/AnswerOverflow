...
import { getMemberCount, toAOChannel, toAOServer } from '~discord-bot/utils/conversions';
...
async function autoUpdateServerInfo(guild: Guild) {
  ...
  const upserted = await upsertServer({
    ...
    update: {
      ...
      approximateMemberCount: getMemberCount(guild) > 0 ? getMemberCount(guild) : undefined,
    },
  });
  registerServerGroup(
    serverWithDiscordInfoToAnalyticsData({
...
export class SyncOnReady extends Listener {
  public async run() {
    ...
    for await (const guild of guilds.values()) {
      ...
      this.container.logger.info(`Syncing guild ${fullGuild.name}`);
      await syncServer(fullGuild);
      activeServerIds.add(fullGuild.id);
    }
